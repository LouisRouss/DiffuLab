...
from diffulab.diffuse.flow import Flow
...
class Trainer:
    ...
    def train(
        ...
        denoiser, dataloader, optimizer = self.accelerator.prepare((denoiser, dataloader, optimizer))  # type: ignore

        if denoiser.context_embedder is not None and not train_embedder:  # type: ignore
            for param in denoiser.context_embedder.parameters():  # type: ignore
                param.requires_grad = False

        tracker = AverageMeter()
        tq_epoch = tqdm(range(self.n_epoch), disable=not self.accelerator.is_main_process)
        for epoch in tq_epoch:
            denoiser.train()  # type: ignore
            ...
            for batch in tq_batch:
                with self.accelerator.accumulate(denoiser):  # type: ignore
                    optimizer.zero_grad()  # type: ignore
                    timesteps = flow.draw_timesteps(self.batch_size)
                    loss = flow.compute_loss(model=denoiser, model_inputs=batch, timesteps=timesteps)  # type: ignore
            ...
            if val_dataloader is not None:
                denoiser.eval()  # type: ignore
                tq_val_batch = tqdm(val_dataloader, disable=not self.accelerator.is_main_process)  # type: ignore
                for val_batch in tq_val_batch:
                    with torch.no_grad():  # type: ignore
                        timesteps = flow.draw_timesteps(self.batch_size)
                        val_loss = flow.compute_loss(model=denoiser, model_inputs=val_batch, timesteps=timesteps)  # type: ignore
                        tracker.update(val_loss.item(), key="val_loss")
                        tq_val_batch.set_description(f"Val Loss: {tracker.avg['val_loss'] :.4f}")
                gathered_val_loss: list[Tensor] = self.accelerator.gather(  # type: ignore
